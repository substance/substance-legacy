...
var Document = function(doc, schema) {
 ...
 _.extend(Document.prototype, _.Events, {
  ...
  checkout: function(ref) {
    ...
    _.each(this.commits(ref), function(op) {
  ...
  undo: function() {
    ...
    if (commit && commit.parent) {
      ...
      this.trigger('ref:updated', 'master', commit.parent);
    } else {
      // No more commits available
      this.reset();
      this.head = null;
      this.trigger('ref:updated', 'master', null);
    }
  },
  ...
  redo: function() {
    var commit = this.commits('master', this.head)[0];

    if (commit) {
      this.checkout(commit.sha);
      this.trigger('ref:updated', 'master', commit.sha);
    }
  },
  ...
  apply: function(operation, options) {
    ...
    if (!options.silent) {
      this.trigger('commit:applied', this.commit(operation));
    }
  },
