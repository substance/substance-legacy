...
var AnnotatedDocument = _.inherits(Document, {
  ...
  getComments: function(node) {
    ...
    _.each(this.comments, function(c) {
      ...
      // Check if comment belongs to node
      var annotation = this.annotations[c.annotation];
      if (annotation && annotation.node === node) comments.push(c);
    }, this);
    return comments;
  },
  ...
  storeAnnotationOp: function(op, scope) {
    ...
    console.log('MEEHE', this.model.annotations[sha]);
