...
app.document.updateSelectedNode({content:""})}else this.$node.hasClass("empty")&&this.$node.text().trim().length>0&&this.$node.removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){var b=_.stripTags(a.$node.html());app.document.updateSelectedNode({name:b})},5)},render:function(){}}),TextEditor=Backbone.View.extend({events:{},initialize:function(){var a=this;this.render();this.$content=this.$("div.content");editor.activate(this.$content);if(this.$content.hasClass("empty")){this.$content.html("<p></p>");
_.fullSelection(this.$content[0])}editor.bind("changed",function(){a.updateNode()});this.$content.bind("blur",function(){a.updateState();a.$content.unbind("blur")})},updateState:function(){if(this.$content.text().trim().length===0){this.$content.html("&laquo; Enter Text &raquo;");this.$content.addClass("empty");app.document.updateSelectedNode({content:"<p></p>"})}else this.$content.hasClass("empty")&&this.$content.text().trim().length>0&&this.$content.removeClass("empty")},updateNode:function(){var a=
this;setTimeout(function(){app.document.updateSelectedNode({content:a.$content.html()})},5)},render:function(){}}),QuoteEditor=Backbone.View.extend({events:{},initialize:function(){function a(){if(document.activeElement===b.$author[0]){if(b.$author.hasClass("empty")){b.$author.html("");_.fullSelection(b.$author[0])}}else if(b.$content.hasClass("empty")){b.$content.html("");_.fullSelection(b.$content[0])}}var b=this;this.$content=this.$(".quote-content").unbind();this.$author=this.$(".quote-author").unbind();
...
(function(){$(function(){window.positionViewActions=function(){document.getElementById("main").offsetTop-(document.body.scrollTop||document.documentElement.scrollTop)<0?$(".view-actions").addClass("docked"):$(".view-actions").removeClass("docked")};positionViewActions();$(window).bind("scroll",positionViewActions);$(window).bind("resize",positionViewActions);app=new Application({el:$("#container"),session:session});editor=new Proper;controller=new ApplicationController({app:this});Backbone.history.start();
var a=false;graph.bind("dirty",function(){if(!a){a=true;setTimeout(function(){$("#sync_state").html("Synchronizing...");console.log(graph.dirtyNodes().keys());graph.sync(function(b,c){a=false;if(!b&&c.length===0){$("#sync_state").html("Successfully synced.");setTimeout(function(){$("#sync_state").html("")},3E3)}else{confirm("There was an error during synchronization. The workspace will be reset for your own safety");window.location.reload(true)}})},3E3)}});graph.bind("conflicted",function(){if(app.document.model){graph.fetch({creator:app.document.model.get("creator")._id,
name:app.document.model.get("name")},{expand:true},function(){app.document.render();app.scrollTo("#document_wrapper")});notifier.notify({message:"There are conflicting nodes. The Document will be reset for your own safety.",type:"error"})}})})})();
