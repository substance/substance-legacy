...
f.reset();f.trigger("changed");f.loadedDocuments[a+"/"+b]=g;app.browser.graph.set("objects",g,f.model);app.toggleView("document");c&&!e&&app.scrollTo(c);if(c&&e){g=graph.get(c.replace(/_/g,"/"));f.selectedNode=g;f.trigger("select:node",f.selectedNode);f.enableCommentEditor(g);$("#"+c+" > .comments-wrapper").show();app.scrollTo(e)}}else $("#document_wrapper").html("Document loading failed")}},error:function(){$("#document_tab").html("&nbsp;&nbsp;&nbsp; Document not found.");$("#document_wrapper").html('<div class="notification error">The requested document couldn\'t be found.</div>');
app.toggleView("document")}})},subscribeDocument:function(){if(!app.username){alert("Please log in to make a subscription.");return false}graph.set(null,{type:"/type/subscription",user:"/user/"+app.username,document:this.model._id});this.model.set({subscribed:true,subscribers:this.model.get("subscribers")+1});this.model._dirty=false;this.render();return false},unsubscribeDocument:function(){var a=this;graph.fetch({type:"/type/subscription",user:"/user/"+app.username,document:this.model._id},function(b,
c){if(c.length!==0){graph.del(c.first()._id);a.model.set({subscribed:false,subscribers:a.model.get("subscribers")-1});a.model._dirty=false;a.render()}});return false},closeDocument:function(){this.model=null;router.navigate(this.app.username);$("#document_wrapper").attr("url","#"+this.app.username);$("#document_tab").hide();app.toggleView("content");this.render()},deleteDocument:function(a){graph.del(a);app.browser.graph.del(a);app.browser.render();$("#document_tab").hide();setTimeout(function(){app.toggleView("browser")},
...
b)){this.selectedNode=graph.get(b);this.trigger("select:node",this.selectedNode);a.stopProp=true;$("#"+this.selectedNode.html_id+" > .comments-wrapper").removeClass("expanded")}a.stopPropagation()},publishDocument:function(){this.model.set({published_on:(new Date).toJSON()});this.render();return false},unpublishDocument:function(){this.model.set({published_on:null});this.render();return false},updateName:function(a){this.model.set({name:a})},addChild:function(a){if(arguments.length===1){var b=$(a.currentTarget).attr("type"),
c=graph.get($(a.currentTarget).attr("node"));b=graph.set(null,{type:b,document:this.model._id})}else{c=graph.get(arguments[1]);b=graph.set(arguments[0].nodeId,arguments[0])}c.all("children").set(b._id,b);c._dirty=true;this.trigger("change:node",c);this.selectedNode=b;this.trigger("select:node",this.selectedNode);return false},addSibling:function(a){if(arguments.length===1)var b=$(a.currentTarget).attr("type"),c=graph.get($(a.currentTarget).attr("node")),e=graph.get($(a.currentTarget).attr("parent")),
d=$(a.currentTarget).attr("destination"),f=graph.set(null,{type:b,document:this.model._id});else{c=graph.get(arguments[1]);e=graph.get(arguments[2]);d=arguments[3];f=graph.set(arguments[0].nodeId,arguments[0])}c=e.all("children").index(c._id);if(d==="after")c+=1;if(b==="/type/section"){b=e.get("children").rest(c);var g=false;b=b.select(function(h){if(!g&&h.type.key!=="/type/section"){e.all("children").del(h._id);return true}else{g=true;return false}});f.set({children:b.keys()})}e.all("children").set(f._id,
f,c);e._dirty=true;this.trigger("change:node",e);this.selectedNode=f;this.trigger("select:node",this.selectedNode);return false},removeNode:function(a){if(arguments.length===1)var b=graph.get($(a.currentTarget).attr("node")),c=graph.get($(a.currentTarget).attr("parent"));else{b=graph.get(arguments[0]);c=graph.get(arguments[1])}c.all("children").del(b._id);graph.del(b._id);c._dirty=true;this.trigger("change:node",c);this.reset();return false}}),Attributes=Backbone.View.extend({initialize:function(){this.el=
"#attributes"},render:function(){app.document.mode==="edit"?this.renderEdit():this.renderShow()},renderShow:function(){var a=app.document.model,b=[];b=a.properties().select(function(c){if(c.expectedTypes[0]==="/type/attribute")return true});$(this.el).html(_.tpl("show_attributes",{attributes:b,doc:a}))},availableAttributes:function(a){return graph.find({"type|=":["/type/attribute"],member_of:"/"+a.type._id.split("/")[2]+"/"+a.key})},renderEdit:function(){var a=this,b=app.document.model,c=[];c=b.properties().select(function(e){if(e.expectedTypes[0]===
"/type/attribute")return true});$(this.el).html(_.tpl("edit_attributes",{attributes:c,doc:b}));$(".attribute-editor").each(function(){var e=$(this).attr("property");graph.get("/type/"+e.split("/")[1]).get("properties",e.split("/")[2]);var d=$(this).attr("key"),f=$(this).hasClass("unique"),g=$(this).attr("type");value=f?app.document.model.get(d).get("name"):_.map(app.document.model.get(d).values(),function(j){return j.get("name")});var h=a.createAttributeEditor(d,g,f,value,$(this));h.bind("input:changed",
function(j){if(j.length<2)return h.updateSuggestions({});$.ajax({type:"GET",data:{member:e,search_str:j},url:"/attributes",dataType:"json"}).success(function(m){graph.merge(m.graph);h.updateSuggestions(m.graph)})});h.bind("changed",function(){var j=[];_.each(h.value(),function(i){var n=graph.find({"type|=":["/type/attribute"],member_of:e,name:i}).first();n||(n=graph.set(null,{type:["/type/attribute"],member_of:e,name:i}));j.push(n._id)});var m={};m[d]=j;app.document.model.set(m);app.document.trigger("changed")})})},
