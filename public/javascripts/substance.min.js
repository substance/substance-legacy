...
render:function(){$(this.el).html(_.tpl("multi_string_editor",{items:this._items}))}});
s.views.DocumentLens=Backbone.View.extend({id:"document_lens",events:{"click a":"scrollTo"},initialize:function(){_.bindAll(this);this.start=this.selectedItem=0;this.windowSize=12;this.height=this.windowSize*30},scrollTo:function(a){var b=$(a.currentTarget).attr("href").slice(1),c=$(a.currentTarget).attr("data-index");app.scrollTo(b);setTimeout(_.bind(function(){this.selectSection(c);this.model.document.prevSection=c},this),40);return false},getBounds:function(){start=Math.max(0,Math.min(this.model.items.length-
this.windowSize,this.selectedItem-(this.windowSize-1)/2));return[start,start+this.windowSize-1]},selectSection:function(a){this.selectedItem=a;this.$(".items .item.selected").removeClass("selected");this.$($(".items .item")[a]).addClass("selected");this.$(".outline .outline-item").removeClass("selected");this.$($(".outline .outline-item")[a]).addClass("selected");this.$(".items").scrollTop(this.getBounds()[0]*30)},render:function(){var a=this.getBounds(),b=this;$(this.el).html(s.util.tpl("document_lens",
{items:this.model.items,bounds:a,selectedItem:this.selectedItem}));this.$(".items").scroll(function(c){c.preventDefault();c.stopPropagation();start=Math.round($(this).scrollTop()/30);$(".outline .outline-item").removeClass("active").each(function(d){d>=start&&d<=start+b.windowSize-1&&$(this).addClass("active")});return false});this.$(".items").scrollTop(a[0]*30);a=Math.min(this.height/this.model.items.length,30);this.$(".outline .outline-item").height(a);return this}});
s.views.Document=Backbone.View.extend({initialize:function(a){function b(d,e){if(e===1)c=[];d.get("children").each(function(g){if(g.type.key==="/type/section"){c.push({id:g._id,html_id:g.html_id,name:g.get("name"),level:e});b(g,e+1)}});return c}_.bindAll(this);this.authorized=a.authorized;this.published=a.published;this.version=a.version;var c;b(this.model,1);this.node=s.views.Node.create({model:this.model,document:this});this.documentLens=new s.views.DocumentLens({model:{items:c,document:this},authorized:this.authorized});
this.settings=new s.views.DocumentSettings({model:this.model,authorized:this.authorized});this.publish=new s.views.Publish({model:this.model,docView:this,authorized:this.authorized});this.invite=new s.views.Invite({model:this.model,authorized:this.authorized});this["export"]=new s.views.Export({model:this.model,authorized:this.authorized});this.subscribers=new s.views.Subscribers({model:this.model,authorized:this.authorized});this.versions=new s.views.Versions({model:this.model,authorized:this.authorized});
graph.bind("dirty",_.bind(function(d){this.updatePublishState();if(d.type._id==="/type/section"){this.calcBounds();setTimeout(_.bind(function(){this.documentLens.model.items=b(this.model,1);this.documentLens.render()},this),2)}},this));this.currentView=null;_.bindAll(this,"deselect","onKeydown");$(document.body).keydown(this.onKeydown);this.bounds||setTimeout(this.calcBounds,400);$(window).scroll(this.markActiveSection)},render:function(){$(this.el).html(s.util.tpl("document",{doc:this.model,authorized:this.authorized}));
$(this.documentLens.render().el).appendTo(this.$("#document"));$(this.node.render().el).appendTo(this.$("#document"));this.authorized&&!this.version&&this.edit();this.$("#document_content").click(this.deselect);return this},remove:function(){$(document.body).unbind("keydown",this.onKeydown);this.$("#document_content").unbind("click",this.deselect);$(this.el).remove();return this},events:{"click .toggle-subscription":"toggleSubscription","click .settings":"toggleSettings","click .publish":"togglePublish",
"click .invite":"toggleInvite","click .subscribers":"toggleSubscribers","click .versions":"toggleVersions","click .export":"toggleExport"},toggleSubscription:function(){this.model.get("subscribed")?unsubscribeDocument(this.model,_.bind(function(){this.$(".action.toggle-subscription a").html("Bookmark");this.$(".tab.subscribers a").html(this.model.get("subscribers"))},this)):subscribeDocument(this.model,_.bind(function(){this.$(".action.toggle-subscription a").html("Remove Bookmark");this.$(".tab.subscribers a").html(this.model.get("subscribers"))},
this));return false},toggleSettings:function(){this.toggleView("settings");return false},togglePublish:function(){this.toggleView("publish");return false},toggleSubscribers:function(){this.toggleView("subscribers");return false},toggleVersions:function(){this.toggleView("versions");return false},toggleInvite:function(){this.toggleView("invite");return false},toggleExport:function(){this.toggleView("export");return false},onKeydown:function(a){a.keyCode===27&&this.deselect()},updatePublishState:function(){$("#document_navigation .publish-state").removeClass().addClass("publish-state "+
getPublishState(this.model))},edit:function(){this.node.transitionTo("write")},deselect:function(){if(this.node){this.node.deselectNode();window.editor.deactivate();this.$(":focus").blur()}},calcBounds:function(){var a=this;this.bounds=[];this.sections=[];$("#document .content-node.section").each(function(){a.bounds.push($(this).offset().top);a.sections.push(graph.get(this.id.replace(/_/g,"/")))})},markActiveSection:function(){var a=this;a.activeSection=function(){var b=0;_.each(a.bounds,function(c,
d){if($(window).scrollTop()>=c-90)b=d});return b}();if(a.activeSection!==a.prevSection){a.prevSection=a.activeSection;a.documentLens.selectSection(a.activeSection)}},resizeShelf:function(){var a=this.currentView?$(this.currentView.el).outerHeight():0,b=a+100;this.$("#document_shelf").css({height:a+"px"});this.$("#document_content").css({"margin-top":b+"px"});this.$("#document_lens").css({top:b+40+"px"})},closeShelf:function(){if(this.currentView){this.currentView.unbind("resize",this.resizeShelf);
$(this.currentView.el).detach();this.currentView=null;this.$(".document.tab").removeClass("selected");this.resizeShelf()}},toggleView:function(a){var b=this[a],c=this.$("#document_shelf");this.$("#document_content");if(this.currentView&&this.currentView===b)return this.closeShelf();this.$(".document.tab").removeClass("selected");$(".document.tab."+a).addClass("selected");b.load(_.bind(function(){b.bind("resize",this.resizeShelf);c.empty();c.append(b.el);this.currentView=b;b.render()},this))}});
s.views.Export=Backbone.View.extend({className:"shelf-content",id:"document_export",render:function(){$(this.el).html(s.util.tpl("document_export",{baseUrl:document.location.href}));this.trigger("resize");return this},load:function(a){a(null)}});
...
function loadComments(a,b){graph.fetch({type:"/type/comment",node:a._id},function(c,d){if(c)return b(c,null);b(null,d.sort(function(e,g){var h=e.value.get("created_at"),i=g.value.get("created_at");return h===i?0:h<i?-1:1}))})}
function createComment(a,b,c,d){window.pendingSync=true;var e=graph.set(null,{type:"/type/comment",creator:"/user/"+app.username,created_at:new Date,content:b,node:a._id,document:a.get("document")._id,version:c?"/version/"+a.get("document")._id.split("/")[3]+"/"+c:null});graph.sync(function(g){window.pendingSync=false;g?d(g,null):d(null,e)})}function removeComment(a,b){window.pendingSync=true;graph.del(a._id);graph.sync(function(c){window.pendingSync=false;b(c)})}
function loadDocument(a,b,c,d){$.ajax({type:"GET",url:c?"/documents/"+a+"/"+b+"/"+c:"/documents/"+a+"/"+b,dataType:"json",success:function(e){if(e.error)d(e,null);else{graph.merge(e.graph);d(null,{version:e.version,authorized:e.authorized,published:e.published,doc:graph.get(e.id)})}},error:function(e){d(e,null)}})}function sortByUpdatedAt(a){return a.sort(function(b,c){var d=b.value.get("updated_at"),e=c.value.get("updated_at");return d===e?0:d>e?-1:1})}
function loadUserProfile(a,b){$.ajax({type:"GET",url:"/documents/"+a,dataType:"json",success:function(c){var d=new Data.Graph(seed);d.merge(c.graph);c=d.find({"type|=":"/type/document"});b(null,{documents:sortByUpdatedAt(c),user:d.get("/user/"+a)})},error:function(c){b(c)}})}
...
function loadExplore(a){var b=(new Data.Graph(seed)).connect("ajax");$.ajax({type:"GET",url:"/networks/recent",dataType:"json",success:function(c){b.merge(c.graph);c=b.find({type:"/type/network"});a(null,{networks:sortByUpdatedAt(c)})},error:function(c){a(c)}})}
...
"",m=0,p=f.length;m<p;m++)k+=c(f[m],v);return k}function o(f){return f.replace(/<[^>]*>/g,"")}function n(f){for(var k="",m=0,p=f.length;m<p;m++)k+=c(f[m],t);return k}function u(f){for(var k=0,m=f.length;k<m;k++)c(f[k],w)}var r=[a],q=null,v={Str:function(f){return f},Emph:i("em"),Strong:i("strong"),Strikeout:l,Superscript:l,Subscript:l,SmallCaps:l,Quoted:function(f){return l(f[1])},Cite:function(f){return l(f[1])},Code:function(f){return"<code>"+f[1]+"</code>"},Space:j(" "),EmDash:j("\u2014"),EnDash:j("\u2013"),
...
{type:"/type/image",url:k,caption:o(l(m)),document:a._id});["Space","LineBreak"].indexOf(f[0])>=0;)f.shift()}if(f.length){e();g("<p>"+l(f)+"</p>")}},CodeBlock:function(f){h();var k=["javascript","python","ruby","php","html","css","haskell","coffeescript","java","c"],m="null";_.each(f[0][1],function(p){p=p.toLowerCase();if(k.indexOf(p)>=0)m=p});f=graph.set(null,{type:"/type/code",language:m,content:s.util.escape(f[1]),document:a._id});d(f)},RawBlock:function(){h()},BlockQuote:function(f){h();f=graph.set(null,
{type:"/type/quote",content:o(n(f)),author:"",document:a._id});d(f)},OrderedList:function(f){e();g(t.OrderedList(f))},BulletList:function(f){e();g(t.BulletList(f))},DefinitionList:function(f){_.each(f,function(k){e();g("<p><strong>"+l(k[0])+"</strong></p>");_.each(k[1],u)})},Header:function(f){h();var k=f[0];for(f=graph.set(null,{type:"/type/section",document:a._id,name:o(l(f[1]))});r.length>k;)r.pop();d(f);r.push(f)},HorizontalRule:function(){h()},Table:function(){h()},Null:function(){h()}},t={Plain:function(f){return this.Para(f)},
Para:function(f){return l(f)},CodeBlock:j(""),RawBlock:j(""),BlockQuote:j(""),OrderedList:function(f){return"<ol>"+_.map(f[1],function(k){return"<li>"+n(k)+"</li>"}).join("")+"</ol>"},BulletList:function(f){return"<ul>"+_.map(f,function(k){return"<li>"+n(k)+"</li>"}).join("")+"</ul>"},DefinitionList:j(""),Header:function(f){return"<strong>"+l(f[1])+"</strong>"},HorizontalRule:j(""),Table:j(""),Null:j("")};u(b[1])}
...
dataType:"json",success:function(a){if(a.status==="error")notifier.notify({message:"An error occured. Check your input",type:"error"});else{graph.merge(a.seed);app.username=a.username;app.render();router.navigate(app.username,true)}},error:function(){notifier.notify({message:"An error occured. Check your input",type:"error"})}}):notifier.notify({message:"Password and confirmation do not match.",type:"error"});return false},initialize:function(){},render:function(){$(this.el).html(s.util.tpl("user_settings",
{user:graph.get("/user/"+app.username)}));return this}});
...
multiline:false,codeFontFamily:'Monaco, Consolas, "Lucida Console", monospace'},d||{});e=e||function(i,j,l,o){var n={};n[j]=l;n.direction=o;updateNode(i,n)};var g=this,h=this.model.get(b);if(h)d.markup?$(a).html(h):$(a).text(s.util.unescape(h));else $(a).html("&laquo; "+c+" &raquo;").addClass("empty");$(a).addClass("editable").click(function(){if(g.state==="write"){window.editor.activate($(a),d);window.editor.bind("changed",function(){e(g.model,b,window.editor.content(),window.editor.direction())})}});
return $(a)},render:function(){this.operationsEl=$(s.util.tpl("operations",{commentCount:this.model.get("comment_count")||"",authorized:this.root.document.authorized})).appendTo(this.el);this.contentEl=$('<div class="content"/>').appendTo(this.el);this.model.get("direction")==="right"&&this.contentEl.css("direction","rtl");if(this.comments)this.commentsEl=$(this.comments.render().el).appendTo(this.el);return this}}),{states:{read:{enter:function(){},leave:function(){}},write:{enter:function(){},leave:function(){}},
move:{enter:function(){$(this.el).addClass("being-moved")},leave:function(a){if(a==="moveTarget")return false;$(this.el).removeClass("being-moved")}},moveTarget:{enter:function(){},leave:function(){}}},subclasses:{},define:function(a,b,c){c=c||{};var d=this.extend(b,c);_.each(_.isArray(a)?a:[a],function(e){this.subclasses[e]=d},this);return d},create:function(a){var b=a.model.type._id,c=this.subclasses[b];if(!c)throw Error("Node has no subclass for type '"+b+"'");return new c(a)}});
