...
Helpers.renderTemplate=_.renderTemplate=function(a,b,c){source=$("script[name="+a+"]").html();return Handlebars.compile(source)(b,c||{})};function getCodeMirrorModeForLanguage(a){return{javascript:"javascript",python:{name:"python",version:3},ruby:"ruby",php:"php",html:"htmlmixed",css:"css",haskell:"haskell",coffeescript:"coffeescript",java:"text/x-java",c:"text/x-csrc","c++":"text/x-c++src",csharp:"text/x-csharp"}[a]||"null"}
function activateCodeMirror(a){var b=function(d){return d.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")};a=$(a);if(!a.data("codemirror")){var c=CodeMirror.fromTextArea(a.get(0),{mode:getCodeMirrorModeForLanguage(a.attr("data-language")),lineNumbers:true,theme:"elegant",indentUnit:2,indentWithTabs:false,tabMode:"shift",readOnly:app.document.mode!=="edit",onFocus:function(){a.trigger("click")},onBlur:function(){c.setSelection({line:0,ch:0},{line:0,ch:0})},onChange:_.throttle(function(){app.document.updateSelectedNode({content:b(c.getValue())})},
500)});setTimeout(function(){c.refresh()},10);a.data("codemirror",c)}return a.data("codemirror")}_.slug=function(a){a=a.replace(/^\s+|\s+$/g,"");a=a.toLowerCase();for(var b=0;b<28;b++)a=a.replace(RegExp("\u00e0\u00e1\u00e4\u00e2\u00e8\u00e9\u00eb\u00ea\u00ec\u00ed\u00ef\u00ee\u00f2\u00f3\u00f6\u00f4\u00f9\u00fa\u00fc\u00fb\u00f1\u00e7\u00b7/_,:;".charAt(b),"g"),"aaaaeeeeiiiioooouuuunc------".charAt(b));return a=a.replace(/[^a-z0-9 -]/g,"").replace(/\s+/g,"-").replace(/-+/g,"-")};
_.scrollTop=function(){return document.body.scrollTop||document.documentElement.scrollTop};_.tpl=function(a,b){source=$("script[name="+a+"]").html();return _.template(source,b)};_.fullSelection=function(a){var b;if(document.createRange){b=document.createRange();b.selectNodeContents(a);a=window.getSelection();a.removeAllRanges();a.addRange(b)}else if(document.selection){b=document.body.createTextRange();b.moveToElementText(a);b.collapse(false);b.select()}};_.prettyDate=function(a){return jQuery.timeago(a)};
...
i,h){function j(n,p){function t(l){if(!(l.nodeType==="/type/section"&&l.level>3))if(l.nodeType==="/type/section"){var r=k.get(l.nodeType);r.push(l);r.sort(function(u,v){return u.level===v.level?0:u.level<v.level?-1:1});k.set(l.nodeType,r)}else if(!k.get(l.nodeType)[0]||l.level>k.get(l.nodeType)[0].level)k.set(l.nodeType,[l])}var q=parseInt($("#"+n.html_id).attr("level"));m=n;n.level=q;q<=3&&o.push(n);if(n.all("children")&&n.all("children").length===0&&i==="after"){var s=n.properties().get("children").expectedTypes;
_.each(s,function(l){t({node:n._id,parentNode:p?p._id:null,nodeType:l,nodeTypeName:graph.get(l).name,insertionType:"child",level:q+1})})}if(p){s=p.properties().get("children").expectedTypes;_.each(s,function(l){t({node:n._id,parentNode:p?p._id:null,nodeType:l,nodeTypeName:graph.get(l).name,insertionType:"sibling",level:q})})}h&&n.all("children")&&n.all("children").length>0&&j(n.all("children").last(),n);return k}var k=new Data.Hash,m=null,o=[];k.set("/type/section",[]);k.set("/type/text",[]);k.set("/type/image",
[]);k.set("/type/resource",[]);k.set("/type/quote",[]);k.set("/type/code",[]);j(g,d);g=m.all("children")&&m.all("children").length===0&&i==="after"?"child":"sibling";return _.tpl("controls",{node:m,insertion_type:g,destination:i,actions:k,path:o})}if(d){b&&$(f(a,"before")).insertBefore($("#"+a.html_id));if(!c||d.types().get("/type/document"))$(f(a,"after",true)).insertAfter($("#"+a.html_id))}else{$("#document .controls").remove();if(!a.all("children")||a.all("children").length===0)$(f(a,"after")).appendTo($("#"+
...
""),level:g})},"/type/code":function(e,f,g){return Helpers.renderTemplate("code",{node:e,comments:e.get("comment_count"),parent:f,edit:app.document.mode==="edit",content:e.get("content"),language:e.get("language"),languageSelect:function(i,h){var j="<select>";_.each(h,function(k){var m=k.toLowerCase();selected=i===m?' selected="selected"':"";j+='<option value="'+m+'"'+selected+">"+k+"</option>"});j+="</select>";return j}(e.get("language"),["JavaScript","Python","Ruby","PHP","HTML","CSS","Haskell",
"CoffeeScript","Java","C","C++","CSharp","Other"]),empty:app.document.mode==="edit"&&(!e.get("content")||e.get("content")===""),level:g})},"/type/question":function(e,f,g){return Helpers.renderTemplate("question",{node:e,comments:e.get("comment_count"),parent:f,edit:app.document.mode==="edit",content:e.get("content"),empty:app.document.mode==="edit"&&(!e.get("content")||e.get("content")===""),level:g})},"/type/answer":function(e,f,g){return Helpers.renderTemplate("answer",{node:e,comments:e.get("comment_count"),
parent:f,edit:app.document.mode==="edit",content:e.get("content"),empty:app.document.mode==="edit"&&(!e.get("content")||e.get("content")===""),level:g})},"/type/image":function(e,f,g){return _.tpl("image",{node:e,comments:e.get("comment_count"),parent:f,edit:app.document.mode==="edit",url:e.get("url"),original_url:e.get("original_url"),level:g,empty:app.document.mode==="edit"&&(!e.get("caption")||e.get("caption")===""),caption:e.get("caption"),transloadit_params:config.transloadit})},"/type/resource":function(e,
f,g){return Helpers.renderTemplate("resource",{node:e,comments:e.get("comment_count"),parent:f,edit:app.document.mode==="edit",url:e.get("url"),level:g,empty:app.document.mode==="edit"&&(!e.get("caption")||e.get("caption")===""),caption:e.get("caption"),transloadit_params:config.transloadit})},"/type/visualization":function(e,f,g){return Helpers.renderTemplate("visualization",{node:e,comments:e.get("comment_count"),parent:f,edit:app.document.mode==="edit",visualization_type:e.get("visualization_type"),
data_source:e.get("data_source"),level:g})}};return{render:function(){return d[a.type._id](a,b,parseInt(c))}}},TOCRenderer=function(a){var b={"/type/document":function(c){content="<ol>";c.all("children").each(function(d){if(d.type.key==="/type/section"){content+='<li><a class="toc-item" node="'+d.html_id+'" href="#'+a.get("creator")._id.split("/")[2]+"/"+a.get("name")+"/"+d.html_id+'">'+d.get("name")+"</a>";content+=b["/type/document"](d);content+="</li>"}});content+="</ol>";return content},"/type/story":function(c){return b["/type/document"](c)},
...
app.document.trigger("changed")}}),SectionEditor=Backbone.View.extend({initialize:function(){this.render();this.$node=$("#"+app.document.selectedNode.html_id+" > .content").attr("contenteditable",true).unbind();editor.activate(this.$node,{placeholder:"Enter Section Name",multiline:false,markup:false});editor.bind("changed",function(){app.document.updateSelectedNode({name:editor.content()})})},render:function(){}}),TextEditor=Backbone.View.extend({events:{},initialize:function(){this.render();this.$content=
this.$("div.content");editor.activate(this.$content,{placeholder:"Enter Text",controlsTarget:$(this.el)});editor.bind("changed",function(){app.document.updateSelectedNode({content:editor.content()})})},render:function(){}}),QuoteEditor=Backbone.View.extend({events:{},initialize:function(){function a(){editor.activate(c.$content,{multiline:false,markup:false,placeholder:"Enter Quote"});editor.bind("changed",function(){c.updateNode({content:editor.content()})})}function b(){editor.activate(c.$author,
{multiline:false,markup:false,placeholder:"Enter Author"});editor.bind("changed",function(){c.updateNode({author:editor.content()})})}var c=this;this.$content=this.$(".quote-content").unbind();this.$author=this.$(".quote-author").unbind();c.$content.bind("click",a);c.$author.bind("click",b);document.activeElement===c.$author[0]?b():a();return true},updateNode:function(a){app.document.updateSelectedNode(a);app.document.trigger("changed")},render:function(){}}),CodeEditor=Backbone.View.extend({events:{},
initialize:function(){var a=activateCodeMirror(this.$("textarea").get(0));a.focus();this.$("select").change(function(){var b=$(this).val();app.document.updateSelectedNode({language:b});a.setOption("mode",getCodeMirrorModeForLanguage(b))})}}),QuestionEditor=Backbone.View.extend({events:{},initialize:function(){this.render();this.$content=this.$(".content");editor.activate(this.$content,{multiline:false,markup:false});editor.bind("changed",function(){app.document.updateSelectedNode({content:editor.content()})})},
...
upload:function(){this.$(".upload-image-form").submit()},initialize:function(){var a=this;this.$caption=this.$(".caption");editor.activate(this.$caption,{placeholder:"Enter Caption",multiline:false,markup:false});editor.bind("changed",function(){app.document.updateSelectedNode({caption:editor.content()})});this.$(".upload-image-form").transloadit({modal:false,wait:true,autoSubmit:false,onProgress:function(b,c){var d=parseInt(b/c*100);d>=0||(d=0);a.$(".image-progress .label").html("Uploading ... "+
d+"%");a.$(".progress-bar").attr("style","width:"+d+"%")},onStart:function(){a.$(".image-progress").show();a.$(".info").hide();a.$(".image-progress .label").html("Uploading ...");a.$(".progress-bar").attr("style","width: 0%")},onSuccess:function(b){if(b.results.web_version&&b.results.web_version[1]&&b.results.web_version[1].url){app.document.updateSelectedNode({url:b.results.web_version[1].url,original_url:b.results.print_version[1].url,dirty:true});app.document.reset();a.$(".progress-container").hide();
a.$(".info").show()}else{a.$(".image-progress .label").html("Invalid image. Skipping ...");a.$(".progress-container").hide();setTimeout(function(){app.document.reset();a.$(".info").show()},3E3)}}})},render:function(){}}),ResourceEditor=Backbone.View.extend({events:{"keydown .resource-url":"update"},resourceExists:function(a,b){var c=new Image;c.onload=function(){b(null)};c.onerror=function(){b("not found")};c.src=a},update:function(){var a=this;setTimeout(function(){var b=a.$(".resource-url").val();
a.resourceExists(b,function(c){if(c)a.$(".status").replaceWith('<div class="status">Invalid URL</div>');else{a.$(".resource-content img").attr("src",b);a.$(".status").replaceWith('<div class="status image">Image</div>');app.document.updateSelectedNode({url:b})}})},500)},initialize:function(){this.pendingCheck=false;this.$caption=this.$(".caption");editor.activate(this.$caption,{placeholder:"Enter Caption",multiline:false,markup:false});editor.bind("changed",function(){app.document.updateSelectedNode({caption:editor.content()})})},
render:function(){}}),UI={};UI.StringEditor=Backbone.View.extend({events:{"change input":"updateValue"},initialize:function(a){var b=this;this._value=a.value;this.bind("changed",function(){b.render()});this.render()},updateValue:function(){this._value=this.$("input[name=value]").val();this.trigger("changed");this.render();return false},value:function(){return this._value},render:function(){$(this.el).html(_.renderTemplate("string_editor",{value:this._value}))}});
...
d.offset();$("html, body").animate({scrollTop:d.top-100},"slow")}});return false},toggleMoveNode:function(){var a=this;$("#document").addClass("move-mode");$(".move-node").hide();var b=$(".content-node .controls");b.find(".placeholder.move").show();b.each(function(){function c(k){if(k.get("children")){var m=false;k.get("children").each(function(o){if(o.type.key==="/type/section"){m||(h+=1);m=true;c(o)}})}}function d(k){if(k===f)j=true;else(k=a.getParent(k))&&d(k);return j}var e=$(this),f=a.selectedNode,
g=a.selectedNode.type.key=="/type/section"?"container-node":"leaf-node",i=0,h=0;c(f);var j=false;e.find(".move-node."+g).each(function(){var k=$(this).hasClass("child")?"child":"sibling",m=parseInt($(this).attr("level")),o=graph.get($(this).attr("node")),n=a.getParent(o);j=false;if(!d(o))if(k==="sibling"){k=n.properties().get("children").expectedTypes;if(_.include(k,a.selectedNode.type.key))if(f.type.key!=="/type/section"?true:m+h<=3){$(this).show();i++}}else{$(this).show();i++}});i===0&&e.find(".placeholder.move").hide();
...
a.html_id),c=graph.get(b.attr("parent"));b=parseInt(b.attr("level"));_.include(a.types().keys(),"/type/document")?$("#document").html((new HTMLRenderer(a,c,b)).render()):$("#"+a.html_id).replaceWith((new HTMLRenderer(a,c,b)).render());this.mode==="edit"&&renderControls(this.model,null,null,null,0);$(".content-node.code textarea").each(function(){var d=activateCodeMirror(this);$(this).data("codemirror",d)})},renderDocument:function(){this.$("#document").html((new HTMLRenderer(this.model,null,0)).render());
this.$("#attributes").show();this.$("#document").show();this.mode==="edit"&&renderControls(this.model,null,null,null,0);$(".content-node.code textarea").each(function(){activateCodeMirror(this)})},documentTypes:function(){var a=[];graph.get("/config/substance").get("document_types").each(function(b,c){a.push({type:c,name:graph.get(c).name})});return a},renderMenu:function(){if(this.model){$("#document_tab").show();$("#document_tab").html(_.tpl("document_tab",{username:this.model.get("creator")._id.split("/")[2],
document_name:this.model.get("name")}))}},init:function(){var a=this;this.unbind("select:node");this.bind("select:node",function(b){a.resetSelection();$("#"+b.html_id).addClass("selected");$("#document").addClass("edit-mode");editor.deactivate();a.renderNodeEditor(b)});this.bind("change:node",function(b){a.renderNode(b)});a.selectedNode=null;$(document).unbind("keyup");$(document).keyup(function(b){b.keyCode==27&&a.reset();b.stopPropagation()});$(document).bind("keydown","alt+down",function(){if(a.selectedNode){var b=
$(".controls[node="+a.selectedNode._id+"][destination=after]");if(b){b.addClass("active");$("#document").addClass("insert-mode")}}});$(document).bind("keydown","right",function(){});$(document).bind("keydown","left",function(){})},newDocument:function(a,b,c){this.model=addEmptyDoc(a,b,c);this.status=null;this.mode="edit";this.authorized=true;$(this.el).show();this.render();this.loadedDocuments[app.username+"/"+b]=this.model._id;this.init();app.browser&&app.browser.query&&app.browser.query.type===
"user"&&app.browser.query.value===app.username&&app.browser.graph.set("nodes",this.model._id,this.model);app.toggleView("document");router.navigate(this.app.username+"/"+b);$("#document_wrapper").attr("url","#"+this.app.username+"/"+b);this.trigger("changed");notifier.notify(Notifications.BLANK_DOCUMENT);return false},loadDocument:function(a,b,c,d,e,f){function g(j){h.model=graph.get(j);if(h.mode==="edit"&&!(head.browser.webkit||head.browser.mozilla)){alert("Your browser is not yet supported. We recommend Google Chrome and Safari, but you can also use Firefox.");
h.mode="show"}if(h.model){h.render();h.init();h.reset();h.trigger("changed");h.loadedDocuments[a+"/"+b]=j;app.browser.graph.set("nodes",j,h.model);app.toggleView("document");d&&!e&&app.scrollTo(d);if(d&&e){j=graph.get(d.replace(/_/g,"/"));h.selectedNode=j;h.trigger("select:node",h.selectedNode);h.enableCommentEditor(j,function(){$("#"+d+" > .comments-wrapper").show();graph.get(e.replace(/_/g,"/"))?app.scrollTo(e):app.scrollTo("comments"+d)})}}else $("#document_wrapper").html("Document loading failed")}
function i(j){if(j==="not_authorized"){$("#document_tab").html("&nbsp;&nbsp;&nbsp; Not Authorized");$("#document_wrapper").html('<div class="notification error">You are not authorized to access this document.</div>')}else{$("#document_tab").html("&nbsp;&nbsp;&nbsp; Document not found");$("#document_wrapper").html('<div class="notification error">The requested document couldn\'t be found.</div>')}app.toggleView("document")}var h=this;$("#tabs").show();$("#document_tab").show();$("#document_tab").html("&nbsp;&nbsp;&nbsp;Loading...");
$.ajax({type:"GET",url:c?"/documents/"+a+"/"+b+"/"+c:"/documents/"+a+"/"+b,dataType:"json",success:function(j){h.version=j.version;h.authorized=j.authorized;h.published=j.published;h.mode=f||(j.authorized&&!c?"edit":"show");if(j.status==="error")i(j.error);else{graph.merge(j.graph);g(j.id)}},error:function(j){i(JSON.parse(j.responseText).error)}})},subscribeDocument:function(){if(!app.username){alert("Please log in to make a subscription.");return false}graph.set(null,{type:"/type/subscription",user:"/user/"+
...
2)return i.updateSuggestions({});$.ajax({type:"GET",data:{member:d,search_str:h},url:"/attributes",dataType:"json"}).success(function(j){graph.merge(j.graph);i.updateSuggestions(j.graph)})});i.bind("changed",function(){var h=[];_.each(i.value(),function(k){var m=graph.find({"type|=":["/type/attribute"],member_of:d,name:k}).first();m||(m=graph.set(null,{type:["/type/attribute"],member_of:d,name:k}));h.push(m._id)});var j={};j[e]=h;app.document.model.set(j);app.document.trigger("changed")})})},createAttributeEditor:function(a,
...
"click .toggle.user-profile":"toggleUserProfile","submit #signup-form":"registerUser","click .toggle.notifications":"toggleNotifications","click .toggle-toc":"toggleTOC","click #event_notifications a .notification":"hideNotifications","click #toc_wrapper":"toggleTOC","click a.open-notification":"openNotification","change #document_name":"updateDocumentName","click a.toggle-recent":"toggleRecent","click a.toggle-subscribed":"toggleSubscribed","click a.toggle-userdocs":"toggleUserDocs","click a.watch-intro":"watchIntro"},
watchIntro:function(){$("#startpage .intro").height("400");$("#startpage .intro .intro-text").fadeOut();setTimeout(function(){$("#startpage .intro .video").html('<video autoplay width="920" height="400" controls><source src="http://substance.io/videos/substance_intro.mp4" type=\'video/mp4; codecs="avc1.42E01E, mp4a.40.2"\'><source src="http://substance.io/videos/substance_intro.ogv" type="video/ogg" /> </video>');setTimeout(function(){$("#startpage .intro .video").fadeIn()},400)},1E3);return false},
toggleRecent:function(){this.recentDocs();return false},toggleSubscribed:function(){this.subscribedDocs();return false},toggleUserDocs:function(){this.userDocs(app.username);return false},userDocs:function(a){app.browser.load({type:"user",value:a});$("#browser_wrapper").attr("url",a);app.browser.bind("loaded",function(){app.toggleView("browser");app.browser.unbind("loaded")});return false},updateDocumentName:function(a){var b=$(a.currentTarget).val();this.checkDocumentName(b,function(c){if(c){app.document.updateName(b);
router.navigate(app.username+"/"+b)}else{$("#document_name").val(app.document.model.get("name"));alert("Sorry, this name is already taken.")}});return false},login:function(){var a=this;this.authenticate($("#login-user").val(),$("#login-password").val(),function(b){if(b)return notifier.notify(Notifications.AUTHENTICATION_FAILED);a.trigger("authenticated")});return false},openNotification:function(a){a=$(a.currentTarget).attr("href");var b=a.replace("#","/").split("/"),c=b[1],d=b[2],e=!b[3]||b[3].indexOf("_")>=
0?null:b[3];app.document.loadDocument(c,d,e,e?b[4]:b[3],e?b[5]:b[4]);$("#document_wrapper").attr("url",a);return false},recentDocs:function(){app.browser.load({type:"recent",value:50});$("#browser_wrapper").attr("url","recent");app.browser.bind("loaded",function(){app.toggleView("browser");app.browser.unbind("loaded")});return false},subscribedDocs:function(){app.browser.load({type:"subscribed",value:50});$("#browser_wrapper").attr("url","subscribed");app.browser.bind("loaded",function(){app.toggleView("browser");
app.browser.unbind("loaded")});return false},searchDocs:function(a){app.browser.load({type:"keyword",value:encodeURI(a)});$("#browser_wrapper").attr("url","search/"+encodeURI(a));app.browser.bind("loaded",function(){app.toggleView("browser")})},toggleStartpage:function(){app.browser.browserTab.render();$("#content_wrapper").html(_.tpl("startpage"));$("#slider").nivoSlider({manualAdvance:true});var a=document.createElement("script"),b=document.getElementsByTagName("script")[0];a.type="text/javascript";
a.async=true;a.src="http://api.flattr.com/js/0.6/load.js?mode=auto";b.parentNode.insertBefore(a,b);app.toggleView("content");return false},showNotifications:function(){this.header.notificationsActive=true;this.header.render()},toggleTOC:function(){if($("#toc_wrapper").is(":hidden")){$("#document .board").addClass("active");$("#toc_wrapper").slideDown();$("#toc_wrapper").css("top",Math.max(_.scrollTop()-$("#document").offset().top,0))}else{$("#document .board").removeClass("active");$("#toc_wrapper").slideUp()}return false},
hideNotifications:function(){graph.find({"type|=":"/type/notification",recipient:"/user/"+app.username}).select(function(a){return!a.get("read")}).each(function(a){a.set({read:true})});this.header.notificationsActive=false;this.header.render()},toggleNotifications:function(){$("#event_notifications").hasClass("active")?this.hideNotifications():this.showNotifications();return false},loadNotifications:function(){var a=this;$.ajax({type:"GET",url:"/notifications",dataType:"json",success:function(b){var c=
{};_.each(b,function(d,e){graph.get(e)||(c[e]=d)});graph.merge(c);a.header.render()}})},toggleUserProfile:function(){var a=this;app.browser.load({type:"user",value:this.username});app.browser.bind("loaded",function(){app.toggleView("browser");$("#browser_wrapper").attr("url",a.username);app.browser.unbind("loaded")})},newDocument:function(){if(!head.browser.webkit&&!head.browser.mozilla){alert("You need to use a Webkit based browser (Google Chrome, Safari) in order to write documents. In future, other browers will be supported too.");
return false}this.content=new NewDocument({el:"#content_wrapper"});this.content.render();this.toggleView("content");return false},scrollTo:function(a){(a=$("#"+a).offset())&&$("html, body").animate({scrollTop:a.top},"slow");return false},toggleUserSettings:function(){this.content=new UserSettings({el:"#content_wrapper"});this.content.render();this.toggleView("content");return false},toggleSignup:function(){$("#content_wrapper").attr("url","register");app.browser.browserTab.render();$("#content_wrapper").html(_.tpl("signup"));
app.toggleView("content");return false},switchTab:function(a){this.toggleView($(a.currentTarget).attr("view"))},toggleView:function(a){$(".tab").removeClass("active");$("#"+a+"_tab").addClass("active");if(!(a==="browser"&&!this.browser.loaded)){$(".view").hide();$("#"+a+"_wrapper").show();$("#header").show();$("#tabs").show();$("#footer").show();setTimeout(function(){router.navigate($("#"+a+"_wrapper").attr("url"))},10);return false}},checkDocumentName:function(a,b){if(RegExp(graph.get("/type/document").get("properties",
"name").validator).test(a)){$.ajax({type:"GET",url:"/documents/"+app.username+"/"+a,dataType:"json",success:function(c){c.status==="error"?b(true):b(false)},error:function(){b(true)}});return false}else b(false)},createDocument:function(){var a=this,b=$("#create_document input[name=new_document_name]").val(),c=_.slug(b);this.checkDocumentName(c,function(d){if(d)a.document.newDocument("/type/article",c,b);else{$("#create_document input[name=new_document_name]").addClass("error");$("#new_document_name_message").html("This document name is already taken.")}});
