...
this[a].text().trim().length>0&&this[a].removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){var b=a.$node.hasClass("empty")?"":_.stripTags(a.$node.html()),c=a.$lead.hasClass("empty")?"":_.stripTags(a.$lead.html());app.document.updateSelectedNode({title:b,lead:c});app.document.trigger("changed")},5)}}),SectionEditor=Backbone.View.extend({events:{"keydown .property":"updateNode"},initialize:function(){var a=this;this.render();this.$node=$("#"+app.document.selectedNode.html_id+
" > .content").attr("contenteditable",true).unbind();if(this.$node.hasClass("empty")){this.$node.html("");_.fullSelection(this.$node[0])}this.$node.unbind("keydown");this.$node.bind("keydown",function(b){return b.keyCode!==13?a.updateNode():false});this.$node.bind("blur",function(){a.updateState()})},updateState:function(){if(this.$node.text().trim().length===0){this.$node.html("&laquo; Enter Section Name &raquo;");this.$node.addClass("empty");app.document.updateSelectedNode({content:""})}else this.$node.hasClass("empty")&&
this.$node.text().trim().length>0&&this.$node.removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){var b=_.stripTags(a.$node.html());app.document.updateSelectedNode({name:b})},5)},render:function(){}}),TextEditor=Backbone.View.extend({events:{},initialize:function(){var a=this;this.render();this.$content=this.$("div.content");editor.activate(this.$content);if(this.$content.hasClass("empty")){this.$content.html("");_.fullSelection(this.$content[0])}editor.bind("changed",function(){a.updateNode()});
this.$content.bind("blur",function(){a.updateState();a.$content.unbind("blur")})},updateState:function(){if(this.$content.text().trim().length===0){this.$content.html("&laquo; Enter Text &raquo;");this.$content.addClass("empty");app.document.updateSelectedNode({content:""})}else this.$content.hasClass("empty")&&this.$content.text().trim().length>0&&this.$content.removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){app.document.updateSelectedNode({content:a.$content.html()})},
5)},render:function(){}}),QuoteEditor=Backbone.View.extend({events:{},initialize:function(){function a(){if(document.activeElement===b.$author[0]){if(b.$author.hasClass("empty")){b.$author.html("");_.fullSelection(b.$author[0])}}else if(b.$content.hasClass("empty")){b.$content.html("");_.fullSelection(b.$content[0])}}var b=this;this.$content=this.$(".quote-content").unbind();this.$author=this.$(".quote-author").unbind();a();this.$content.bind("focus",a);this.$author.bind("focus",a);this.$content.bind("blur",
function(){console.log("blurred from quote");b.updateState("$content")});this.$author.bind("blur",function(){console.log("blurred from Author");b.updateState("$author")});this.$content.unbind("keydown");this.$content.bind("keydown",function(c){return c.keyCode!==13?b.updateNode():false});this.$author.unbind("keydown");this.$author.bind("keydown",function(c){return c.keyCode!==13?b.updateNode():false})},updateState:function(a){if(this[a].text().trim().length===0){if(a==="$content"){this[a].html("&laquo; Enter Quote &raquo;");
app.document.updateSelectedNode({content:""})}else{this[a].html("&laquo; Enter Author &raquo;");app.document.updateSelectedNode({author:""})}this[a].addClass("empty")}else this[a].hasClass("empty")&&this[a].text().trim().length>0&&this[a].removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){var b=a.$content.hasClass("empty")?"":_.stripTags(a.$content.html()),c=a.$author.hasClass("empty")?"":_.stripTags(a.$author.html());app.document.updateSelectedNode({content:b,author:c})},
5)},render:function(){}}),CodeEditor=Backbone.View.extend({events:{},initialize:function(){var a=this;this.render();this.$content=this.$(".content");editor.activate(this.$content);$(".proper-commands").hide();if(this.$content.hasClass("empty")){this.$content.html("");_.fullSelection(this.$content[0])}editor.bind("changed",function(){a.updateNode()});this.$content.bind("blur",function(){a.updateState();a.$content.unbind("blur")})},updateState:function(){if(this.$content.text().trim().length===0){this.$content.html("&laquo; Enter code &raquo;");
this.$content.addClass("empty");app.document.updateSelectedNode({content:""})}else this.$content.hasClass("empty")&&this.$content.text().trim().length>0&&this.$content.removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){app.document.updateSelectedNode({content:a.$content.html()})},5)},render:function(){}}),QuestionEditor=Backbone.View.extend({events:{},initialize:function(){var a=this;this.render();this.$content=this.$(".content");editor.activate(this.$content);$(".proper-commands").hide();
if(this.$content.hasClass("empty")){this.$content.html("");_.fullSelection(this.$content[0])}editor.bind("changed",function(){a.updateNode()});this.$content.bind("blur",function(){a.updateState();a.$content.unbind("blur")})},updateState:function(){if(this.$content.text().trim().length===0){this.$content.html("&laquo; Enter Question &raquo;");this.$content.addClass("empty");app.document.updateSelectedNode({content:""})}else this.$content.hasClass("empty")&&this.$content.text().trim().length>0&&this.$content.removeClass("empty")},
updateNode:function(){var a=this;setTimeout(function(){app.document.updateSelectedNode({content:a.$content.html()})},5)},render:function(){}}),AnswerEditor=Backbone.View.extend({events:{},initialize:function(){var a=this;this.render();this.$content=this.$(".content");editor.activate(this.$content);$(".proper-commands").hide();if(this.$content.hasClass("empty")){this.$content.html("");_.fullSelection(this.$content[0])}editor.bind("changed",function(){a.updateNode()});this.$content.bind("blur",function(){a.updateState();
a.$content.unbind("blur")})},updateState:function(){if(this.$content.text().trim().length===0){this.$content.html("&laquo; Enter Answer &raquo;");this.$content.addClass("empty");app.document.updateSelectedNode({content:""})}else this.$content.hasClass("empty")&&this.$content.text().trim().length>0&&this.$content.removeClass("empty")},updateNode:function(){var a=this;setTimeout(function(){app.document.updateSelectedNode({content:a.$content.html()})},5)},render:function(){}}),ImageEditor=Backbone.View.extend({events:{},
initialize:function(){this.render();$("#upload_image").transloadit({modal:false,wait:true,autoSubmit:false,onProgress:function(a,b){percentage=(a/b*100).toFixed(2)||0;$("#upload_progress").attr("style","width:"+percentage+"%");$("#image_progress_legend").html("<strong>Uploading:</strong> "+percentage+"% complete</div>")},onError:function(a){alert(a.error+": "+a.message);$("#progress_container").hide()},onStart:function(){$("#progress_container").show()},onSuccess:function(a){app.document.updateSelectedNode({url:a.results.resize_image[0].url,
dirty:true});$("#progress_container").hide()}})},render:function(){this.$(".node-editor-placeholder").html(Helpers.renderTemplate("edit_image",app.editor.model.selectedNode.data))}}),ApplicationController=Backbone.Controller.extend({routes:{"^(?!search)(.*)/(.*)$":"loadDocument","^(?!search)(.*)/(.*)/(.*)$":"loadDocument",":username":"userDocs","^search/(.*)$":"searchDocs"},loadDocument:function(a,b,c){app.browser.load({type:"user",value:a});app.document.loadDocument(a,b,c);$("#document_wrapper").attr("url",
"#"+a+"/"+b);$("#browser_wrapper").attr("url","#"+a);return false},userDocs:function(a){if(!a)if(app.username)a=app.username;else return app.toggleStartpage();a==="recent"?app.browser.load({type:"recent",value:50}):app.browser.load({type:"user",value:a});$("#browser_wrapper").attr("url","#"+a);app.browser.bind("loaded",function(){app.toggleView("browser");app.browser.unbind("loaded")});return false},searchDocs:function(a){app.searchDocs(a);return false}}),UI={};
...
$(".attribute-editor").each(function(){var d=$(this).attr("property");graph.get("/type/"+d.split("/")[1]).get("properties",d.split("/")[2]);var j=$(this).attr("key"),k=$(this).hasClass("unique"),l=$(this).attr("type");value=k?app.document.model.get(j).get("name"):_.map(app.document.model.get(j).values(),function(u){return u.get("name")});var m=a.createAttributeEditor(j,l,k,value,$(this));m.bind("input:changed",function(u){if(u.length<2)return m.updateSuggestions({});$.ajax({type:"GET",data:{member:d,
search_str:u},url:"/attributes",dataType:"json"}).success(function(i){graph.merge(i.graph);m.updateSuggestions(i.graph)})});m.bind("changed",function(){var u=[];_.each(m.value(),function(v){var w=graph.find({"type|=":["/type/attribute"],member_of:d,name:v}).first();w||(w=graph.set(null,{type:["/type/attribute"],member_of:d,name:v}));u.push(w._id)});var i={};i[j]=u;app.document.model.set(i);app.document.trigger("changed")})})},createAttributeEditor:function(a,b,c,d,j){switch(b){case "string":return c?
this.createStringEditor(a,d,j):this.createMultiStringEditor(a,d,j);case "number":break;case "boolean":break}},createMultiStringEditor:function(a,b,c){return new UI.MultiStringEditor({el:c,items:b})},createStringEditor:function(a,b,c){return new UI.StringEditor({el:c,value:b})}}),AddCriterion=function(a,b){this.app=a;this.options=b};
